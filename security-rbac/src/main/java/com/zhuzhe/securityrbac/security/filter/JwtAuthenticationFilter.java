...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    log.info("address={},browser={}", address, browser);
    ...
    try {
      ...
      if (userPrincipal!=null){
        ...
        if (StringUtils.isNotBlank(userPrincipal.getUsername())
          ...
          if (CollectionUtils.isNotEmpty(roles))
            authorities = roles.stream().map(Role::getKey).map(SimpleGrantedAuthority::new).toList();
          authenticationToken = ApiAuthenticationToken.authenticated(userPrincipal, authorities);
        }
        SecurityContextHolder.getContext().setAuthentication(authenticationToken);
      }
      log.info("[JwtAuthenticationFilter]doFilterInternal处理结束,进入下一个过滤器.");
      ...
      if (e instanceof SecurityException) {
        new ObjectMapper()
            .writeValue(
                response.getWriter(), Map.of(((SecurityException) e).getCode(), e.getMessage()));
      } else {
        e.printStackTrace();
